...
class FacadeSampler(nn.Module):
    """
    A sampler that implements the top-nsigma truncation.

    It is named Facade instead of TopNSigma because it simply follows the facade design pattern.
    It must be used with the ugly hack in hack.py.
    """
    ...
    def forward(
        ...
        mean, std = logits.mean(dim=-1, keepdim=True), logits.std(dim=-1, keepdim=True)
